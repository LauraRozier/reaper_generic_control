...
public:
	...
	void learnTrackBankButton(const std::string& button)
	{
		bool ok = getInput("press the " + button + " button", button + " button");

		if (ok)
			_bank[button] = _input_id;
	}
	...
	void learn()
	{
		...
		std::cout << "Does your control surface have track buttons ? (Y/N)" << std::endl;
		answer = _getch();

		if (answer == 'y' || answer == 'Y')
		{
			learnTrackBankButton("Track Back");
			learnTrackBankButton("Track Forward");
		}
	...
	rapidjson::Value serializeBank(rapidjson::MemoryPoolAllocator< >& alloc)
	{
		rapidjson::Value result(rapidjson::kObjectType);

		for (const auto& button : _bank)
			result.AddMember(jsonString(button.first, alloc), button.second, alloc);
	...
	void serialize()
	{
		...
		if (!_bank.empty())
			doc.AddMember("Control", serializeBank(alloc), alloc);
